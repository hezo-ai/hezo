...
test('can create a team with Startup template and see auto-created agents', async ({ page }) => {
	await page.goto('/');
...
test('new team page shows template selection', async ({ page }) => {
	await page.goto('/');
	await authenticate(page);
	await page.goto('/teams/new');
...
test('post-create redirect lands on projects page with create dialog open', async ({ page }) => {
	...
	await expect(page).toHaveURL(/\/teams\/[^/]+\/projects$/, { timeout: 15000 });
});
