...
test('inline run comment shows terminate button and confirms before posting', async ({ page }) => {
...
test('terminate button is hidden for finished runs', async ({ page }) => {
	await authenticate(page);
	const setup = await setupTask(page);
...
test('terminate button renders on mobile viewport', async ({ page }) => {
