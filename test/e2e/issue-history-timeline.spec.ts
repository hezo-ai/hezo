...
test('status changes and cross-issue mentions appear as system entries on the timeline', async ({
	...
	const targetRes = await page.request.post(`/api/teams/${team.id}/issues`, {
		headers,
	...
	const sourceRes = await page.request.post(`/api/teams/${team.id}/issues`, {
		headers,
	...
	const targetUrl = `/teams/${team.slug}/projects/${project.slug}/issues/${target.identifier.toLowerCase()}`;
	...
	await page.request.post(`/api/teams/${team.id}/issues/${source.id}/comments`, {
		headers,
...
test('title renames appear as system entries on the timeline', async ({ page }) => {
	...
	await page.request.patch(`/api/teams/${team.id}/issues/${issue.id}`, {
		headers,
		data: { title: 'Renamed ticket' },
	});

	const issueUrl = `/teams/${team.slug}/projects/${project.slug}/issues/${issue.identifier.toLowerCase()}`;
