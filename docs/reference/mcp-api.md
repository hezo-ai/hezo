...
| `check_frequency` | `daily` \| `weekly` \| `monthly` | No | How often the goal is re-assessed once created (default daily). Pick by how often the measurement meaningfully changes: daily for fast-moving monitoring, weekly for steady work streams, monthly for slow-moving outcomes. Checks recur indefinitely — this is a cadence, not a deadline. |
