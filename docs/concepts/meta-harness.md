...
- **A uniform completeness check on every run.** When an agent decides it's finished, Hezo
  ...
  supports it, which matters most for models that wouldn't hold that line on their own.
  (OpenCode and Grok Build can't support the hook yet, and Hezo has not wired it for
  Antigravity, so runs on those three rely on the model alone.)
