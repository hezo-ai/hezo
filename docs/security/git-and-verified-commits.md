...
credentials that make that possible **never enter the agent's sandbox**, and the commits
your agents make land as **Verified** on GitHub.
...
and no redirect URL to configure. On connect, the project's public key is **automatically
registered** on your GitHub account as a *signing* key, so commits show the Verified
badge. Subsequent
...
If a push is rejected anyway, the failure is written into the run's log with the actual
error from git, so you can see exactly which repository and account were refused.
...
the sandbox - Daytona, for instance, blocks SSH on every port.
...
often. Because Hezo pushes on every commit, a branch that runs CI on each push will build
more often than it would with a single end-of-run push; scope CI to pull requests if that
becomes noisy.

An automatic push never interrupts the agent's commit; the commit lands locally either way.
But if the push itself is rejected, the agent sees the error immediately and the run's log
carries it too, so a repository whose commits are not reaching GitHub is visible without
...
- **The run is marked failed**, even if the agent otherwise finished cleanly. Work that never
  reached GitHub is not a completed run, and marking it failed is what puts it in front of
  you.
...
happens the repository stops keeping up with the remote.
