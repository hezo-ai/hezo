...
pre_marker_generated_config_complete() {
	local path="$1" normalized prefix database asset expected
	[[ -f "${path}" ]] || return 1
	...
	normalized="$(
		sed -E \
			...
			-e 's#^([[:space:]]*)url: ".*",$#\1url: "<database-url>",#' \
			-e 's#^([[:space:]]*)poolSize: ([1-9]|[1-9][0-9]|100),$#\1poolSize: <pool-size>,#' \
			-e 's#^([[:space:]]*)assetStorage: \{ url: ".*" \},$#\1assetStorage: { url: "<asset-storage-url>" },#' \
			"${path}"
		printf '\034'
	)"
	prefix="$(cat <<'EOF'
...
if [[ -n "${CONFIG_READY}" && -f "${LEGACY_ENV}" ]] &&
	{ [[ -n "${LEGACY_ENV_CARRIED}" ]] ||
		{ grep -Fqx "${GENERATED_CONFIG_START_MARKER}" "${CONFIG_FILE}" &&
			grep -Fqx "${GENERATED_CONFIG_COMPLETE_MARKER}" "${CONFIG_FILE}"; } ||
		pre_marker_generated_config_complete "${CONFIG_FILE}"; }; then
