...
Per-run cost is computed in `agent-stream-parser.ts`. Every runtime parser now **prefers a cost the runtime reports itself** (`total_cost_usd` / `cost_usd` / `cost`, found via `findReportedCostUsd`) over the pricing table, falling back to `price()` (the `model_pricing` table via `PricingService`) only when no cost is reported — one shared `resolveCostCents(reported, fallback)` applied across Claude Code, Codex, Gemini, and the generic (OpenCode) parsers. A reported number already reflects the provider's real billing (including any time-of-day/peak pricing), so it's authoritative; the table stays the fallback for runtimes/endpoints that don't report one. Whether a given provider actually reports a cost is an empirical question — measure it with the probe below, not by assumption.

**Confirmed:** Claude Code reports `total_cost_usd` for **every** provider on its runtime — native **Anthropic**, plus the third-party Anthropic-compatible endpoints **DeepSeek** (`api.deepseek.com/anthropic`), **Z.ai** (`api.z.ai/api/anthropic`), and **Kimi** (`api.moonshot.ai/anthropic`), all three including their real (e.g. peak/off-peak) billing — so those runs are priced from run output, and table-side peak/off-peak modeling is **not** needed for billing. **OpenAI** (Codex) emits token usage but **no** dollar cost, so it prices from the table. (Gemini/OpenRouter not yet verified.)
