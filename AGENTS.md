...
- **Adding/changing an AI provider** (`AiProvider` enum + `PROVIDER_RUNTIME_ADAPTERS`) → besides `.dev/architecture.md` and the provider docs, update the **cost probe** (`packages/server/src/services/cost-probe.ts`): it auto-covers a new provider via the shared `RUNTIME_*` maps, but add its `PROBE_DEFAULT_MODELS` entry (a cheap model) and run `bun run cost-probe --provider <slug>` to check the pricing feeds carry sane rates for its models (the probe prints reported-vs-table); add a curated rate (`services/pricing/curated-rates.ts`) if the feeds are wrong or lack the model. See **Provider cost probe** below.
...
The judge prompt body (`STOP_HOOK_RULES` in `stop-hook-prompt.ts`) is identical across every runtime that runs it, so changes to the rules apply everywhere. The judge models are hardcoded per provider (Sonnet for Anthropic, gpt-4o-mini for OpenAI, gemini-1.5-flash for Google, `kimi-k2.7-code` for Kimi) and intended to be revisited after dogfooding. For the **file-mount** subscription providers (Codex / Gemini OAuth flows) the helper script has no API key in env and fails open — exits silently, the agent stops normally. **Anthropic subscription** is the exception: it runs via `CLAUDE_CODE_OAUTH_TOKEN` (a `claude setup-token` value injected as env), so Claude Code's native `type: "prompt"` judge still fires — there is no helper script to fail open.

OpenCode takes the task prompt as a CLI **argument** rather than on stdin (`opencode run <message>`), so the runner sets `HEZO_PROMPT_MODE=arg` (see `RUNTIME_PROMPT_DELIVERY`) and the exec wrapper appends `"$(cat $HEZO_PROMPT_FILE)"` instead of redirecting stdin.

## Cost: always priced from the table

Per-run cost is computed in `agent-stream-parser.ts`, **always** from the `model_pricing` table (`price()` via `PricingService`) using the four token buckets (regular input, cache read, cache creation, output). Runtimes also emit their own dollar figures (`total_cost_usd` / `cost_usd` / `cost`), but those are **ignored in every parser** — they are client-side estimates from the CLI's built-in rate card, and for third-party Anthropic-compatible endpoints that card belongs to the wrong provider. Verified 2026-07-03: Claude Code priced DeepSeek runs (`deepseek-v4-pro[1m]`) at Anthropic Sonnet long-context rates, ~18x DeepSeek's real billing (a run the dashboard billed ~$0.20 was reported as $3.67); the same mechanism applies to Z.ai and Kimi. An earlier claim here that reported costs "reflect real billing" was wrong — the probe had only confirmed the *field exists*, never that its value matches the provider's bill.

Table accuracy is therefore what matters. Rates come from two public feeds (llm-prices.com primary, LiteLLM secondary — see `services/pricing/feed.ts`) with two override layers on top: **curated built-in rates** (`services/pricing/curated-rates.ts`, hand-verified against official pricing pages; they win over both feeds in every merge — the snapshot seed and live refreshes alike) and operator **manual rows** (win over everything). Both feeds mis-list DeepSeek v4 (v4-pro 4x high, cache-read 10–40x high), which is why the curated layer exists — when a feed value is confirmed wrong against the provider's official pricing page, add a curated entry rather than editing `litellm-snapshot.json` (kept verbatim upstream). An unknown model prices to $0 with a one-time warning — fail-low, never fail-high. Runs persist their cache split on `heartbeat_runs.cache_read_tokens` / `cache_creation_tokens` (subsets of the all-buckets `input_tokens` aggregate) so costs stay auditable.

## Provider cost probe

`scripts/cost-probe.ts` (`bun run cost-probe`) is the diagnostic that keeps the table-pricing policy honest. Per provider it answers **two** questions: does a real run emit a cost field at all, and how does that figure compare to what the table computes for the same tokens (`reported $X vs table $Y (N.Nx)`)? A ratio far from 1 for a runtime pricing **its own provider's** models means the table entry is stale — verify against the official pricing page and add a curated rate. A large ratio on a third-party Anthropic-compatible endpoint is expected (wrong rate card) and is precisely why reported figures are ignored. Validating the table against the provider's billing dashboard remains a manual step. The probe drives the **real Docker run path**: spins up the `hezo/agent-base` image, `docker exec`s the actual runtime CLI against the provider's real endpoint with a real key — env + argv assembled from the same `@hezo/shared` adapter maps as `buildRuntimeInvocation`. Provider traffic goes direct, so no egress proxy/CA/DB is needed — just Docker, the image (`--build` builds it if missing), outbound egress to the provider, and a key.
...
- **Keep it current with the provider roster.** Because the invocation is built from the shared `RUNTIME_*` / `PROVIDER_RUNTIME_ADAPTERS` maps, a new provider is auto-covered — except for its `PROBE_DEFAULT_MODELS` entry (a cheap model to keep probes near-free) in `packages/server/src/services/cost-probe.ts`. Add that and run the probe for the new provider to check its table rates. The pure pieces (invocation assembly + cost extraction + table comparison) are unit-tested in `packages/server/test/cost-probe.test.ts`.
- Some runtimes need credential/config files beyond env vars; the probe stages the minimal subset in-container before the run (Codex `auth.json` under `CODEX_HOME`). When adding a provider on a new runtime, check whether it authenticates from env alone or needs similar staging.
