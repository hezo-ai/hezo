...
- `bun run test:daytona` — conformance suites against a live Daytona account (`HEZO_DAYTONA_API_KEY`). Manual only: it provisions billable sandboxes and refuses to start under `CI`. `HEZO_DEEPSEEK_API_KEY` additionally enables `describeAgentCliConformance` (a real coding-CLI run against a real provider, once per fixture `modelProviders` entry - each entry names the runtime, so a non-default CLI like Prime Agent is only exercised where one says so). `bun run test:live` runs every `test/live/` fixture.
