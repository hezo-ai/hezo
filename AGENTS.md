...
**Confirmed via the probe:** **DeepSeek** (Claude Code runtime, `api.deepseek.com/anthropic`) **does** return `total_cost_usd` — including its peak/off-peak billing — so DeepSeek runs are priced from run output, and table-side peak/off-peak modeling is **not** needed for billing. (Codex/Gemini emit token usage but no dollar cost today, so they price from the table; Z.ai is unverified.)
