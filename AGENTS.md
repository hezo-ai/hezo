...
OpenCode, Grok and Kimi Code take the task prompt as a CLI **argument** rather than on stdin, so the runner sets `HEZO_PROMPT_MODE=arg` (`RUNTIME_PROMPT_DELIVERY`) and the exec wrapper appends `"$(cat $HEZO_PROMPT_FILE)" < /dev/null` - the redirect is load-bearing, since an exec's stdin is a pipe nothing closes and a CLI that reads it hangs with no output. Kimi Code additionally gets **no auto-approve flag** — `--yolo`/`--auto`/`--plan` are mutually exclusive with `--prompt`; `-p` already applies the `auto` permission policy and the injected `[permission.rules]` covers the rest.
