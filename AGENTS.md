...
- `--pattern <substring>` filters by file path across all tiers and is repeatable (matches are OR-ed); `--package <server|web|shared>` restricts the vitest run; `--concurrency <n>` sets workers (default 10); `--shard <i>/<n>` runs one shard; `--bail` stops on first failure. `--bun-native` / `--skip-bun-native` force the Bun tier on or off against what `--pattern`/`--shard` would select.
...
- `.dev/` — internal engineering docs and the home for the rationale this file omits. `architecture.md` is the consolidated **descriptive** reference; alongside it sit the **prescriptive** guides — `adding-a-container-backend.md`, `adding-a-chat-channel.md`, `writing-migrations.md`, `writing-agent-prompts.md` — `bun-issues.md`, the register of the production runtime's divergences from Node, and `mcp-cli-efficiency.md`, the measured token savings of the connector-tool CLI. Which of them a given piece of writing belongs in is the routing table at the top of this file.
...
**Every agent CLI is pinned, and a new one arrives pinned.** An `ARG <NAME>_VERSION`, an install that references it, and a `--version` call that fails the build. An unpinned CLI changes a run's behaviour with no commit and no signal. `agent-cli-pins.test.ts` fails the suite on an unpinned or single-arch install; `.github/workflows/cli-drift.yml` reports weekly what has fallen behind, into one tracking issue. **Pinning is not upgrading**: pin at whatever `latest` currently resolves to, and treat a bump as its own change — each CLI's Dockerfile comment says what to re-verify first. Antigravity is the exception in shape only: its installer takes no version, so it is pinned by URL + SHA512, the version asserted from `agy --version`. **A binary artifact is pinned once per architecture**, selected on `TARGETARCH`: only the release build builds arm64, so a single-arch URL fails there and nowhere earlier.
