...
External chat avenues to the CEO (Telegram + Slack now, Discord/WhatsApp later) are built on a **channel-adapter abstraction + registry** so a new app is one file, not a core change. The `ChatChannelAdapter` interface and registry live in `packages/server/src/services/chat-channels/`; the manager (`chat-session-manager.ts`), the generic inbound webhook route (`routes/chat-webhooks.ts` → `ingestInboundEvent`), the conversation model, and the web thread switcher are all **channel-agnostic** — they resolve a channel only through the registry, never by branching on a platform name.
...
- **Assistant/DM mode** (`kind='mirror'`) — a private chat with the bot is a real-time CEO thread mirrored with the web chatbox. Identity-allowlist gated. Telegram; Slack DMs.
- **Group/coworker mode** (`kind='coworker'`) — the CEO is invited into an existing group channel and responds to @-mentions with platform history as ephemeral context, replying in-thread. **Channel invite is the authorization** (no identity gate; a link only enriches attribution). Never mirrored: origin binding only, invisible in the web chatbox, no WS broadcasts, turns queue instead of interrupting, no compaction/auto-title (the operator's long-term chat memory stays out of group prompts). Slack channels.

To add a channel:

1. Add a `ChatChannel` enum value in `packages/shared/src/types/common.ts` **and** an additive `ALTER TYPE chat_channel ADD VALUE` migration (with a data-preservation test). *Telegram/WhatsApp were in the enum from the start; Slack shipped via migration 038; Discord will need one.*
2. Implement a `ChatChannelAdapter` (`chat-channels/<channel>.ts`): `parseInbound` (raw event → `InboundChatEvent`), `deliver` (a message → platform thread), optional `start`/`stop` (webhook registration, or a persistent transport like Slack's Socket Mode client), optional `createThread`/`closeThread` + `supportsThreads` (mirror-thread hosting), optional `parseClose` (platform thread-closed → external id), optional `promptToLink`/`validateConfig`. **Group mode is the optional capability trio** `parseGroupMention`/`supportsGroupMode`/`fetchThreadContext` (the adapter owns the platform history fetch and its filtering; the core owns prompt formatting via `formatGroupContextBlock`). Register it in `buildChatChannelRegistry` (`chat-channels/index.ts`).
3. Inbound transport: webhook channels flow through the generic route → `ingestInboundEvent` (DM) — group mentions go through the **second, deliberately separate ingest path `ingestGroupMentionEvent`** (`chat-channels/ingest-group.ts`); never overload `ingestInboundEvent` with group semantics. A socket-transport adapter pushes parsed events through the `InboundEventSink` on its deps instead (same ingest functions, wired at startup).
...
5. Ship a registry-contract test (parse → `InboundChatEvent` shape, thread create/close no-op safety) plus the channel's own unit tests, and the **parity** checks below; a group-capable adapter also gets coworker-semantics coverage (crib `packages/server/test/chat-group-ingest.test.ts`: origin-only binding, reply-to-origin, no broadcasts, ephemeral context never persisted).

**Channel parity is an invariant for `kind='mirror'` conversations — every real-time chat channel mirrors the same logical thread.** (Coworker conversations are deliberately single-binding and exempt.) A mirror conversation is one logical thread with a **binding per channel** (`chat_conversation_bindings`); the manager/registry own the mirroring and a new adapter never re-implements it. Any adapter that can host threads MUST implement `createThread`/`closeThread` and report `supportsThreads()` so that: **(a)** creating a thread on any surface auto-creates it on every mirror-capable channel (`ChatSessionManager.setupBindings` → `registry.mirrorableChannels()` → each adapter's `createThread`); **(b)** messages sync **both ways** — the operator's message and the CEO's reply fan out to every binding, with **origin-exclusion** so a channel never re-echoes a message it just delivered (`mirrorMessage` / `mirrorUserMessage`); and **(c)** closing on any surface closes it everywhere (`closeConversation` closes every external binding; `parseClose` + `closeConversationByExternalThread` handle the platform→app direction). A channel that **cannot** host threads (a DM-only surface) omits `createThread`/`supportsThreads` and degrades gracefully to a single thread — it never breaks parity for the others. When you add a channel, verify (a)/(b)/(c) with a mirroring test (see `packages/server/test/chat-mirroring.test.ts`); do **not** add per-channel mirroring logic to the manager.

**Do not touch** the manager, `ingestInboundEvent`/`ingestGroupMentionEvent`, the generic webhook route, or the conversation/identity/bindings schema — if a new channel forces a change there, close the gap in the abstraction instead. `chat-channels/slack.ts` + `slack-socket.ts` are the worked persistent-transport example; `.dev/discord-chat-adapter.md` is the deferred Discord blueprint (a true-fanout gateway that, unlike Slack Socket Mode, needs a single-instance ownership lease).
