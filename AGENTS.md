...
- `bun run test --coverage` — instrument the vitest (server, web, and shared) and Bun-native tiers with V8 coverage; writes lcov/json to `packages/<pkg>/coverage` (plus `packages/server/coverage-bun` for the Bun tier). lcov reports are normalized before upload — `SF:` paths rewritten to be repo-root-relative so they map across the monorepo, and the vitest reports scoped to `src/` only. The Bun report is scoped more narrowly, to `src/services/egress/` (the proxy TLS-MITM + secret-substitution path it actually drives): `bun test` boots the whole Hono app, so V8 instruments every transitively-imported src file and emits a mostly-zero line for each, and because Node-V8 and Bun-V8 disagree on which physical lines are "executable" those Bun-only zero lines don't reconcile against the vitest tier's covered lines when Coveralls unions the flags — left unscoped they inflate the denominator and drag the merged total down ~14pts. Scoping keeps the Bun flag purely additive (its unique value is `proxy.ts`, which the Node tier can't fully drive). Off by default so normal runs stay fast. Composes with `--package`/`--shard`. CI uploads each shard/tier's lcov to Coveralls as a parallel build (unique `flag-name` each — `backend-vitest-*`, `backend-bun`, `web-*`, `shared`); a final `coverage-finished` job closes the build so Coveralls merges them into one total. Playwright is not yet instrumented.
