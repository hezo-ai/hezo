...
| `conformance/git.ts` | that a container on this backend can actually clone: a real git host reached over the transport Hezo builds (the assertion whose absence let SSH-over-443 ship to a backend that drops it), plus a clone whose remote carries a placeholder succeeding against a server that demands the real token, with the token nowhere the container can read |

**A fixture registers the set, never individual suites.** `describeContainerBackendConformance(fixture, harness)` (`conformance/index.ts`) is the *only* call a backend's entry point makes. That is what makes "every suite, every adapter" structural rather than customary, in both directions: a new adapter gets the whole set from one line and cannot ship having quietly wired four of six, and a suite added to `conformance/index.ts` reaches every existing adapter without any fixture being edited. Registering suites one by one - which is how this started - is the shape where coverage silently diverges between backends, and a fixture missing a suite is *green*.

Neither direction is something the compiler can see, so `conformance-coverage.test.ts` asserts both: every file in `conformance/` is imported **and called** by the aggregate, and every fixture (found by its import of the aggregate, not by filename) calls that and nothing else. Adding a suite therefore fails the build until it is in the set. **When you add a container service, add its fixture under `test/live/` (paid, manual) or `test/bun/` (free, CI) and call the aggregate** - and when you add a suite, add it to `conformance/index.ts` in the same change.
