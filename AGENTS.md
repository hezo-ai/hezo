...
- `bun run test --shard <index>/<count>` — run one vitest shard (e.g. `1/3`); CI fans `test-backend` (3 shards) and `test-integration` (3 shards) across runners this way; the pure-logic `shared` package runs unsharded as a single `test-shared` job. The Bun-native tier runs only on shard 1. Composes with `--package`/`--concurrency`.
...
- **Adding/changing an AI provider** (`AiProvider` enum + `PROVIDER_RUNTIME_ADAPTERS`) → besides `.dev/architecture.md` and the provider docs, update the **cost-source probe** (`packages/server/src/services/cost-probe.ts`): it auto-covers a new provider via the shared `RUNTIME_*` maps, but add its `PROBE_DEFAULT_MODELS` entry (a cheap model) and run `bun run cost-probe --provider <slug>` to record whether its runtime returns cost in run output. See **Provider cost-source probe** below.
...
- **Grok** (xAI): the Grok Build `grok` CLI reads `~/.grok/config.toml` (Codex-shaped). The completeness judge is wired as a `[[hooks.Stop]]` command script (`buildGrokJudgeScript`) that calls xAI's OpenAI-compatible Chat Completions at `api.x.ai` with `XAI_API_KEY` (judge model `grok-4-fast`). The `grok` CLI is in beta and its exact hook-config schema / Stop stdin payload are undocumented, so — like Kimi — the script probes candidate message fields and fails open when none is present (or when no key is set). See `mcp-injectors/grok.ts` and `buildGrokJudgeScript`. **[confirm the hook schema against the installed CLI.]**
...
## Cost: prefer reported, fall back to the table

Per-run cost is computed in `agent-stream-parser.ts`. Every runtime parser now **prefers a cost the runtime reports itself** (`total_cost_usd` / `cost_usd` / `cost`, found via `findReportedCostUsd`) over the pricing table, falling back to `price()` (the `model_pricing` table via `PricingService`) only when no cost is reported — one shared `resolveCostCents(reported, fallback)` applied across Claude Code, Codex, Gemini, and the generic (OpenCode/Kimi/Grok) parsers. A reported number already reflects the provider's real billing (including any time-of-day/peak pricing), so it's authoritative; the table stays the fallback for runtimes/endpoints that don't report one. Whether a given provider actually reports a cost is an empirical question — measure it with the probe below, not by assumption.

**Confirmed:** Claude Code reports `total_cost_usd` for **every** provider on its runtime — native **Anthropic**, plus the third-party Anthropic-compatible endpoints **DeepSeek** (`api.deepseek.com/anthropic`) and **Z.ai** (`api.z.ai/api/anthropic`), the latter two including their real (e.g. peak/off-peak) billing — so those runs are priced from run output, and table-side peak/off-peak modeling is **not** needed for billing. **OpenAI** (Codex) emits token usage but **no** dollar cost, so it prices from the table. **Kimi** and **xAI/Grok** emit **neither** cost nor token usage in headless mode (Kimi: assistant message + a session-resume hint; Grok: a `text` event + an `end` event), so their runs currently can't be priced from output *or* the table (0 tokens) — a known gap. Their `/usage` slash command is **interactive-TUI-only** — not handled in headless `kimi -p` / `grok -p` (probed: Kimi replies "*`/usage` isn't a recognized command here*"; Grok emits only an `end` event) — so it can't feed headless pricing either. (Gemini/OpenRouter not yet verified.)

## Provider cost-source probe

`scripts/cost-probe.ts` (`bun run cost-probe`) answers, per provider, **does a real agent run return a usable dollar cost in its output, or must we price it from the table?** It drives the **real Docker run path**: spins up the `hezo/agent-base` image, `docker exec`s the actual runtime CLI against the provider's real endpoint with a real key — env + argv assembled from the same `@hezo/shared` adapter maps as `buildRuntimeInvocation` — and scans the JSONL for a reported cost. Provider traffic goes direct, so no egress proxy/CA/DB is needed — just Docker, the image (`--build` builds it if missing), outbound egress to the provider, and a key.
