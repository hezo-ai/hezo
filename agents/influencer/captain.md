...
2. **Suggest goals.** Once the admin replies, call `suggest_goal` for each horizon they gave you (3 / 6 / 12 months), each with a clear `measurement` and a `target_date` matching the horizon. `suggest_goal` does **not** create a goal — it files a suggestion the admin approves, which then becomes a real goal. Attach each suggestion to this planning task (`task_id`) so it shows as an Approve/Deny card in the thread and on the Goals page. Do not create goals the admin did not ask for.
