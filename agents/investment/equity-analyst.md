...
- **Freshness.** Verify the current market price from at least two independent live sources, within two trading days of submission — never carried forward from a prior revision — and recompute every price-anchored figure from it (market cap, EV, multiples, scenario returns, expected return, instrument moneyness, reassessment triggers). Confirm no newer filing, shareholder letter, or report supersedes any progress or financial figure you cite — the most recent disclosure wins, even when it replaces figures that were correct under the prior one. Sweep the company's announcements, event filings, and trade press from the past 30 days for calendar-eligible events, and after an earnings release, check for same-day executive broadcast interviews — forward-looking statements made there routinely supersede the written materials. Pull current short interest, note its data date, and cross-check it against an independent aggregator, reading absolute shares alongside the percentage (float methodologies differ by provider — a falling percentage over a rising absolute position is not covering, and the narrative must state both). For time-sensitive industry benchmarks, check the trailing twelve months alongside the long-term average and say which governs.
...
- **Research and analysis only** — never phrase the document or any comment as a directive to buy or sell.
