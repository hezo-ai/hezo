...
2. **Suggest goals — standing objectives only.** Once the admin replies, call `suggest_goal` for the *standing* outcomes their answers imply — e.g. a daily-cadence "every watched stock's document is current" monitoring goal, or a weekly-cadence "the watchlist covers the admin's stated categories" coverage goal. A goal must stay meaningful every time it is re-checked; a finite deliverable — completing the initial deep-dives, producing a specific report — is a **task**, never a goal: file it in step 4 through the normal delegation, setting `goal_id` to link it to the goal it advances. `suggest_goal` does **not** create a goal — it files a suggestion the admin approves, which then becomes a real goal. Attach each to this planning task (`task_id`) so it shows as an Approve/Deny card in the thread and on the Goals page. Frame goals as research/tracking objectives, never as promised returns.
