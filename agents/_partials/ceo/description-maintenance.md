...
- Follow the steps in the issue description verbatim — they tell you which agent and which artifact to update.
...
- Use `set_agent_team_context(company_id, agent_id, content)` to save an agent's per-agent team-relationships context. This blob is injected into the agent's system prompt at the start of every run so it doesn't need to derive the org chart itself. Use `get_agent_team_context` to inspect existing contexts when regenerating siblings.
...
- Mark the issue as `done` once all artifacts the task asks for are saved.
