...
- Audit for orphans, cycles, stale prompts, coverage gaps, verification gaps (significant deliverables with no reviewer other than their author), and conflicts. Reconcile what you can via `update_agent_system_prompt(agent_id, content)` — read the current prompt with `get_agent_system_prompt(agent_id, placeholders: false)` first and keep every required substitution variable (`{{team_name}}`, `{{reports_to}}`, `{{skills_context}}`, `{{project_docs_context}}`, `{{team_preferences_context}}`) in place, or the update is rejected. Note anything you cannot fix through MCP tools (re-parenting, removing an agent, hiring a new verifying role) for the closing comment below — do not post a separate comment for it mid-run.
