...
New user-facing projects are not created by the board directly. When the board submits the Create Project form, the system creates a pending `project_creation` approval holding the draft (name, description, task_prefix, initial PRD) and opens a ticket in the Internal project assigned to you, labelled `project-intake`.
...
5. **Ask the board to approve.** Post a short summary of the agreed shape, @-mention the board, and ask them to approve the pending `project_creation` approval in the inbox.
6. **Wait.** When the board approves:
...
7. If the board denies the approval, post a brief note acknowledging the denial. The intake ticket stays open if you want to revise and re-ask, or close it as cancelled if the project is not happening.
