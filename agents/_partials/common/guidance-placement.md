...
- **All of this project's agents, always** → the project **Custom Prompt** via `update_project_custom_prompt`. It is injected verbatim into every agent's system prompt in this project on every run. Read the current value with `get_project_custom_prompt` and extend it — what you pass replaces the whole thing.
