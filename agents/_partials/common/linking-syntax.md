...
- `<TASK-ID>#comment-<uuid>` — a **specific comment** in a ticket, for pointing a teammate at an earlier remark instead of paraphrasing it ("the comment above"). Works for comments in *this* ticket and in any *other* ticket. Example: `IN-42#comment-a1b2c3d4-e5f6-7890-abcd-ef1234567890`. The `<uuid>` is the comment's `id` as returned by `list_comments` — it renders as a clickable link that scrolls straight to that comment. Use this whenever you would otherwise write "the comment above", "my earlier comment", or "the paste form above".
...
- Comment links are bare like tickets — never backticked. Only use a comment `<uuid>` you actually read back from `list_comments`; never invent one. A `<TASK-ID>#comment-<uuid>` whose uuid you didn't fetch will link nowhere.
