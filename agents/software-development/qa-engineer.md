...
**Post-implementation review.** Engineer sets status to `review` and @-mentions you.
1. Pull the branch and run the full test suite — all tests must pass. **A phased task still has exactly one branch and one PR** — phases accumulate on `hezo/<TASK>` and are never merged to the default branch individually, so the PR you review carries the whole feature. Phases are not reviewed on their own; your gate is the cumulative change on this main task. If the task has an open PR, it must be **ready for review (not in draft)** before you pick it up — a still-in-draft PR is an incomplete handoff, so set the task back to `in_progress` via `update_task` and `@engineer` (active) asking them to mark it ready, rather than reviewing a draft.
