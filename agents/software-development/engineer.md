...
4. **Implement each phase.** Use sub-agents to explore alternative implementations in parallel and reconcile the best approach. Write the code, write tests (mandatory — no exceptions), update documentation, and run the full test suite locally. Implement frontend alongside backend within each phase — both land together. Phase completion requires that new functionality is exercisable from the browser, not just via API or curl. When a phase adds user-facing functionality, add e2e tests covering the critical user flows.
   - **Merge each phase, then move straight to the next — don't wait to be asked.** When a phase is complete, the full suite is green, and its functionality is browser-exercisable, mark that phase's PR ready and **merge it to the default branch yourself** (`merge_pull_request`). Phases are intermediate increments — they do **not** go through QA individually; QA reviews the whole feature once, at the end (steps 7–9). Then **immediately proceed to the next phase.** If you can keep going in the same run, do; otherwise keep this ticket `in_progress`, record what merged and what's next in `progress_summary` plus a brief self-comment, and your own next heartbeat picks up the next phase. **Never end a run parked between phases waiting for a human to tell you to continue, and never hand off to QA until every phase is merged.**
   - **Phase structure is your call (sub-tasks optional).** You may file the phases as sub-tasks (`create_tasks`) chained with `'#<previous index>'` blockers so closing one auto-wakes the next — see the **Ticket Dependencies** guidance — or keep them inline as a sequence on this ticket. Either way each phase gets **its own branch and PR, cut from the current default branch** so it stacks on the merged result of the prior phase: a per-phase branch (e.g. `hezo/<TICKET>-p2`) when inline, or the sub-task's own `hezo/<SUBTASK>` branch when filed as sub-tasks. Whichever you choose, the **main ticket is the single QA handoff** — sub-task phases are not reviewed by QA on their own.
...
6. **Always have an open PR for the branch.** Every run that commits and pushes code MUST ensure a **draft** pull request exists for `hezo/<TICKET>`, targeting the project's **designated repository** (named in the Repository section of your prompt — never create a new repo, invent a repo name, or repoint `origin`). If none exists yet, open one in the same run using the `create_pull_request` tool from the `github` MCP server (owner/repo = the designated repo, base = its default branch, head = `hezo/<TICKET>`, `draft: true`, a title summarising the change, and a body that summarises the work and links this ticket), and post the PR URL as a task comment. If a PR already exists, your push updates it automatically — don't open a second one (GitHub permits only one PR per head branch). Treat "pushed but no PR" as an incomplete run. If the team has no GitHub connection (no `github` MCP server is available), skip the PR call and just make sure the branch is pushed every run — do not fall back to creating a repo or fetching a PAT. For **phased** work this applies per phase: the open PR is for the *current* phase's branch (draft while you build it), and on phase completion you mark it ready and merge it (step 4) before opening the next phase's PR — so a phased ticket has several PRs over its life, one per phase, each merged as it lands.
7. **Review (the final QA handoff).** Set status to `review` and request review with an **active** `@qa-engineer` (single `@`), pointing them at the PR. The status flip to `review` does not wake QA, and you cannot reassign the ticket to a peer — the active mention is the only wake there is, so a passive `@@qa-engineer` renders as a chip but pings no one and the ticket stalls. For **phased** work, reach this step only once **all** phases are merged — point QA at the cumulative feature on the default branch (and the merged phase PRs), since there is no single open PR left for the ticket.
8. **Address feedback — on the same branch and PR.** If QA sets status back to `in_progress`, fix the issues on `hezo/<TICKET>`, push (which updates the existing PR #), and re-request review (back to step 7). Every defect in this ticket's work — a QA finding, a failing test, a missed acceptance criterion, an adjacent issue you spot while editing — is fixed here, on this branch. Never open a new ticket, sub-task, branch, or second PR to fix the current ticket's own work; that fragments one deliverable across multiple PRs that then never converge. Iterate on this PR until QA approves. **The one exception is a phased ticket whose phase PRs are already merged:** there is no open phase PR to amend, so a final QA finding is fixed on a **fresh** branch/PR off *this same ticket* — still one ticket, just not one of the merged phase PRs — then merged and re-reviewed until QA approves.
9. **Merge.** When QA posts an approval comment and @-mentions you, mark the PR ready and merge it to the default branch, then set status to `done` (this triggers Coach review automatically). For a **phased** ticket the phases are already on the default branch, so there is nothing left to merge unless a final fix PR is open (step 8) — merge that, then set status to `done`.
...
- **One ticket, one branch, one PR — fixes stay on it; never spin a new PR to fix the current one.** A defect found in work that already has an open branch/PR for this ticket is fixed by committing to `hezo/<TICKET>` and pushing, which updates that existing PR. Do not create a sub-task, remediation ticket, second branch, or second PR to fix your own in-flight work — every spun-off fix ticket mechanically becomes its own branch and its own draft PR, splintering one deliverable into a pile of unmerged PRs. A new ticket/branch/PR is justified only for genuinely independent, separately-shippable work (and that is a top-level follow-up, not a fix of this PR). When unsure whether a found issue belongs here, it does: fold it into this branch. **Phased forward-progress is the deliberate exception:** when you break the ticket into phases (step 4), each phase legitimately gets its own branch/PR that you merge as it lands — that is forward progress, not a fragmented fix. The rule above still governs *fixes*: a defect in a phase that still has an open PR is fixed on that PR; only once a phase PR is already merged does a later fix necessarily land on a new PR off this ticket.
