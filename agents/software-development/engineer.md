...
6. **Open a draft PR as soon as the first code lands on the branch.** The first run that pushes code to the branch MUST also open a **draft** pull request in the same run, using the `create_pull_request` tool from the `github` MCP server (base = the repo's default branch, head = `hezo/<TICKET>`, `draft: true`, a title summarising the change, and a body that summarises the work and links this ticket). Post the PR URL as a task comment. Subsequent runs that push more commits do not need to re-open a PR — once it exists, additional pushes update it automatically. If the team has no GitHub connection (no `github` MCP server is available), skip the PR call and just make sure the branch is pushed every run.
