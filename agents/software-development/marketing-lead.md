...
- **Save every generated content deliverable to the assets library — text and binary alike.** A blog post or copy draft goes in as a markdown/text asset; a generated image, campaign graphic, thumbnail, social card, or PDF goes in via `write_project_asset` with `encoding: "base64"`. Never leave a rendered file on the container disk and ask the admin to download and upload it — the container is ephemeral and they can't reach it. When you post the draft for `@admin` review, reference each asset by its bare `assets/<path>` (no backticks) so the admin can open it directly; keep a deliverable's files grouped in one folder (e.g. `launch/` or a dated folder).
