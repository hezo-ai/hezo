...
describe('retiredBudgetFieldError', () => {
	it('refuses a dollar field and names the token field that replaced it', () => {
		const message = retiredBudgetFieldError({ monthly_budget_cents: 3000, title: 'x' });
		expect(message).toContain('monthly_budget_cents -> monthly_budget_tokens');
		expect(message).toContain('tokens, not dollars');
	});

	it('names every retired field a body carries', () => {
		const message = retiredBudgetFieldError(
			Object.fromEntries(Object.keys(RETIRED_BUDGET_FIELDS).map((k) => [k, 0])),
		);
		for (const [cents, tokens] of Object.entries(RETIRED_BUDGET_FIELDS)) {
			expect(message).toContain(`${cents} -> ${tokens}`);
		}
	});

	it('passes a body with only token fields', () => {
		expect(retiredBudgetFieldError({ monthly_budget_tokens: 5 })).toBeNull();
		expect(retiredBudgetFieldError(null)).toBeNull();
	});
});

describe('window floors', () => {
	it('minWeeklyTokens is daily × 7, or 0 when daily is unlimited', () => {
		expect(minWeeklyTokens(100)).toBe(700);
		expect(minWeeklyTokens(0)).toBe(0);
	});

	it('minMonthlyTokens takes the larger of the daily/weekly implied floors', () => {
		expect(minMonthlyTokens(100, 0)).toBe(3042); // ceil(100 * 365/12)
		expect(minMonthlyTokens(0, 1000)).toBe(4334); // ceil(1000 * 52/12)
...
describe('validateBudgetWindows', () => {
	...
	it('flags a weekly budget below the daily-implied floor', () => {
		const v = validateBudgetWindows({
			daily_budget_tokens: 100,
			weekly_budget_tokens: 500,
			monthly_budget_tokens: 0,
		});
		expect(v).toHaveLength(1);
		expect(v[0]).toMatchObject({ field: 'weekly_budget_tokens', minTokens: 700 });
	});
	...
	it('flags a monthly budget below the weekly-implied floor', () => {
		...
		expect(v).toHaveLength(1);
		expect(v[0]).toMatchObject({ field: 'monthly_budget_tokens', minTokens: 4334 });
...
describe('normalizeBudgetWindowsUp', () => {
	it('raises enabled longer windows up to their floors', () => {
		...
		).toEqual({ daily_budget_tokens: 100, weekly_budget_tokens: 700, monthly_budget_tokens: 0 });
	});
	...
	it('feeds the daily floor into the monthly when weekly is disabled', () => {
		...
		).toEqual({ daily_budget_tokens: 100, weekly_budget_tokens: 0, monthly_budget_tokens: 3042 });
	});
