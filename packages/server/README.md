...
| `--auto-install-updates` | `false` | Install staged updates automatically: gracefully restart onto a downloaded-and-verified newer release without the web UI's "Install & restart", deferring while agent runs are in flight. Only effective where in-app auto-update is available (self-managed binary, not in a container); the instance restarts locked unless a master key was provided at startup (env: `HEZO_AUTO_INSTALL_UPDATES`) |
