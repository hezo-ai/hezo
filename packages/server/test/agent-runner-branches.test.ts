...
describe('buildProviderEnv', () => {
	...
	it('stamps GEMINI_API_KEY for a Google api-key credential', () => {
		...
		// Antigravity authenticates the Gemini API from GEMINI_API_KEY (with the
		// modelProvider marker its adapter writes); it needs no workspace-trust env.
		expect(env).toEqual(['GEMINI_API_KEY=AIza-google']);
	});
});
