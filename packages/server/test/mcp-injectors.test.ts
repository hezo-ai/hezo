...
describe('prime-agent adapter', () => {
	const adapter = MCP_ADAPTERS[AgentRuntime.PrimeAgent];
	const PA_HOME = '/workspace/.hezo/subscription/prime-agent-anthropic/run-1';
	const HOMES = { hostHomeDir: PA_HOME, containerHomeDir: PA_HOME };
	const fileNamed = (injection: { files: readonly McpInjectionFile[] }, suffix: string) =>
		injection.files.find((f) => f.hostPath.endsWith(suffix));
	...
	it('writes settings.json referencing the bearer by env var, never inlining it', () => {
		const injection = adapter.build([HEZO_DESCRIPTOR], HOMES);
		const settings = fileNamed(injection, 'settings.json');
		...
		expect(parsed.mcpServers.hezo.bearerTokenEnvVar).toBe('HEZO_MCP_BEARER_TOKEN_HEZO');
		expect(settings?.contents).not.toContain(TOKEN);
		// No env var turns telemetry off, so it has to be stated in the file.
		expect(parsed.telemetry.enabled).toBe(false);
		expect(injection.envEntries).toContain(`HEZO_MCP_BEARER_TOKEN_HEZO=${TOKEN}`);
	});

	it('generates a Python skill package per server and points --skill at it', () => {
		// The load-bearing part of this adapter: a settings.json entry alone
		// registers the endpoint host-side and gives the agent nothing. MCP runs
		// Python-side in the kernel, so a server is only reachable once a package
		// subclassing McpIntegration is loaded as a skill.
		const injection = adapter.build([HEZO_DESCRIPTOR], HOMES);
		expect(injection.cliArgs).toEqual(['--skill', `${PA_HOME}/skills/hezo`]);

		const doc = fileNamed(injection, 'skills/hezo/SKILL.md');
		expect(doc?.contents).toContain('name: hezo');

		const mod = fileNamed(injection, 'skills/hezo/src/hezo/__init__.py');
		expect(mod?.mode).toBe(0o600);
		expect(mod?.contents).toContain('from rlm import McpIntegration');
		expect(mod?.contents).toContain(`url = ${JSON.stringify(URL)}`);
		// The bearer travels as an env var NAME. The value never reaches a file the
		// agent can read, which is what keeps the placeholder substitution intact.
		expect(mod?.contents).toContain('bearer_token_env = "HEZO_MCP_BEARER_TOKEN_HEZO"');
		expect(mod?.contents).not.toContain(TOKEN);
	});

	it('sanitises a connector slug into an importable module name', () => {
		const injection = adapter.build(
			[{ kind: 'http', name: 'my-srv.v2', url: URL, bearerToken: TOKEN }],
			HOMES,
		);
		expect(injection.cliArgs).toEqual(['--skill', `${PA_HOME}/skills/my_srv_v2`]);
		const mod = fileNamed(injection, 'skills/my_srv_v2/src/my_srv_v2/__init__.py');
		// The class name is derived too, and the server keeps its real name so tool
		// discovery still addresses the right endpoint.
		expect(mod?.contents).toContain('class MySrvV2(McpIntegration)');
		expect(mod?.contents).toContain('server = "my-srv.v2"');
	});

	it('drops a stdio server rather than writing one the CLI discards', () => {
		// Prime Agent's host drops non-HTTP entries outright, so emitting one would
		// look configured and do nothing.
		const injection = adapter.build(
			[{ kind: 'stdio', name: 'local-srv', command: '/usr/bin/srv', args: [], env: {} }],
			HOMES,
		);
		const parsed = JSON.parse(fileNamed(injection, 'settings.json')?.contents ?? '') as {
			mcpServers?: Record<string, unknown>;
		};
		expect(parsed.mcpServers).toBeUndefined();
		expect(injection.cliArgs).toEqual([]);
	...
	it('writes an executable judge script carrying the shared rules', () => {
		...
		const judge = fileNamed(injection, 'stop-hook-judge.mjs');
		expect(judge?.mode).toBe(0o700);
		expect(judge?.contents).toContain(STOP_HOOK_JUDGE_MODEL_ANTHROPIC);
		expect(judge?.contents).toContain(JSON.stringify(STOP_HOOK_RULES).slice(1, 60));
		// The gate gets no stdin, so the final message comes from the session log.
		expect(judge?.contents).toContain('PRIME_AGENT_CODING_AGENT_DIR');
		expect(judge?.contents).toContain('.jsonl');
		// No stop_hook_active either, so the loop guard is an on-disk marker.
		expect(judge?.contents).toContain('.hezo-stop-blocked');
	});

	it('still writes settings.json and the judge with no connectors at all', () => {
		const injection = adapter.build([], HOMES);
		expect(fileNamed(injection, 'settings.json')).toBeDefined();
		expect(fileNamed(injection, 'stop-hook-judge.mjs')).toBeDefined();
		expect(injection.cliArgs).toEqual([]);
		expect(injection.envEntries).toEqual([]);
	});
});
