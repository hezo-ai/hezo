...
async function callTool(
	...
	return callMcpTool(app, bearer, toolName, args);
}
...
describe('MCP project progress + goal run activity', () => {
	...
	// The summary replaces whatever was there: the Captain is told to include everything that
	// should remain, so a second call is not a merge.
	it('replaces the whole summary on each call', async () => {
		const agent = await mintAgentToken(db, masterKeyManager, captainAgentId, teamId, null, {
			projectId,
		});
		await callTool(agent.token, 'update_project_progress', {
			project: projectSlug,
			summary: '**Billing is the long pole.** Onboarding is done.',
		});
		await callTool(agent.token, 'update_project_progress', {
			project: projectSlug,
			summary: '**Billing shipped.**',
		});
		...
		expect(body.summary).toBe('**Billing shipped.**');
		expect(body.summary).not.toContain('long pole');
	});
