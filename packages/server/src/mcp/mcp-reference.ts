...
export const TOOL_DOC_META: Record<string, ToolDocMeta> = {
	...
	create_hire_proposal: {
		...
		auth: "Captain only; the proposal is filed on the Captain's team and surfaces as a pending approval for the admin.",
	},
	...
	read_project_doc: {
		category: 'Project docs & assets',
		returns:
			'`{ filename, content }` (the full markdown body), or `{ error }` if the file is not found.',
	},
	write_project_doc: {
		category: 'Project docs & assets',
