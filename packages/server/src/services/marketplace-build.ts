...
export interface TeamManifest {
	...
	summary: string;
	/**
	 * Discovery vocabulary — the words someone would type when they want this
	 * team. Authored here (by hand for Hezo's default teams; generated by Hezo
	 * when a user publishes a team) rather than derived by the picker, so recall
	 * is a property of the team's data and needs no code change to improve.
	 * Required: a team nobody can find is a team nobody uses.
	 */
	keywords: string[];
 ...
 *
 * `keywords` is excluded too, and for the same reason: a team's version drives the
 * "update available" reconcile that re-runs hiring against every project already
 * on that team. Keywords change nothing a reconcile would act on — they only
 * affect which teams the New Project picker surfaces — so retuning them must not
 * bump the version. The picker reads the live catalog, so improved keywords take
 * effect on the next fetch regardless of version.
 */
...
export function buildTeamDef(
	...
	// Not part of `content` — keywords are discovery metadata and stay out of the hash.
	const keywords = normalizeKeywords(manifest.keywords);
	const keywordsErr = teamKeywordsError(keywords);
	if (keywordsErr) throw new Error(`Team "${manifest.slug}": ${keywordsErr}`);
