...
5. This planning ticket is the epic for the plan itself. The server will not let it move to \`done\` while any sub-task is open, so it stays open while research / PRD / design / spec execute. Once those sub-tasks reach a terminal status (\`done\` or \`cancelled\`) and the top-level work tickets have been created, move this task to \`done\`; the Coach reviews it after the post-mortem but it stays \`done\`. Trying to flip it to \`done\` early will fail with a "sub-task(s) still open" error. If you get a ticket's parent wrong, fix it with \`update_task\` — set \`parent_task_id\` to move it, or to an empty string to promote it to top level. Never cancel a mis-filed ticket and re-file it as a new one.
