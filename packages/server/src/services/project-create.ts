...
5. This planning ticket is the epic for the plan itself. The server will not let it move to \`done\` while any sub-task is open, so it stays open while research / PRD / spec / design execute. Once those sub-tasks reach a terminal status (\`done\` or \`cancelled\`) and the top-level work tickets have been created, move this task to \`done\`; the Coach reviews it after the post-mortem but it stays \`done\`. Trying to flip it to \`done\` early will fail with a "sub-task(s) still open" error.
