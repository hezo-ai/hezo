...
- **A ticket waiting on an answer is not complete — ask BEFORE closing, never close-then-ask.** If an active mention you posted asking for a response is still unanswered on this ticket — a question to a teammate (\`@<slug>\`) or to the admin (\`@admin\`) — do NOT set the ticket \`done\`: keep it \`in_progress\` or move it to \`review\` and end your turn; the reply wakes you automatically. Closing first and asking after (even seconds later) is the same failure — a terminal ticket reads as finished, so nobody treats it as awaiting anything. The server enforces the admin half: \`update_task(status: "done")\` is rejected while an \`@admin\` question on the ticket has no later human reply (only a human can close through it). If the question becomes moot, say so in a comment, then close.
