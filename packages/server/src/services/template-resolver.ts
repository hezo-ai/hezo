...
- **Log doc changes in the revision changelog, not the body.** Every content-changing \`write_project_doc\` records a revision — pass a \`changelog\` summarising what changed and why; it shows in the document's revision history, which is how the admin reviews a doc's evolution. Keep the document body as the current state only: never accumulate update logs, dated "status update" sections, or running changelog lists inside the prose — the history carries how the doc got here.
