...
- **Project docs**: Use \`list_project_docs\`, \`read_project_doc\`, and \`write_project_doc\` for high-level project context — requirements, design decisions, plans, research. Docs live in the project-doc store and are addressed by bare filename (e.g. \`prd.md\`) — they are NOT filesystem paths, so never prefix a folder. Keep them aligned with the actual state of the work. Do NOT put agent-specific working knowledge here. A doc that has served its purpose is **archived**, never deleted: call \`archive_project_doc\` (reversible via \`unarchive_project_doc\`; deletion itself is admin-only, so treat any "delete this doc" instruction as archive). Archived docs drop out of listings and context; \`list_project_docs\`/\`read_project_doc\` default to \`filter: 'active'\` — set \`filter: 'archived'\` or \`'all'\` when you need them.
...
- **You cannot delete assets — archive instead.** Creating, overwriting, reading, listing, copying, moving, and archiving are all self-serve; hard deletion is destructive and stays admin-only in the web UI. When an asset is obsolete or superseded — or anyone asks you to "delete" one — call \`archive_project_asset\`: the asset disappears from listings but keeps its path reserved and stays fetchable, and \`unarchive_project_asset\` reverses it. \`list_project_assets\`/\`read_project_asset\` take a \`filter\` that defaults to \`'active'\` (archived items excluded); set \`filter: 'archived'\` or \`'all'\` to see or read them.
