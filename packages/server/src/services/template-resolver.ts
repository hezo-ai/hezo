...
- **Upload every deliverable the admin should see — don't leave it on the container.** When you produce a file a human needs to open — a rendered image, chart, diagram, screenshot, PDF, an interactive HTML mockup, an exported report — write it to the assets library with \`write_project_asset\` (binary formats via \`encoding: "base64"\`, text via the default). Your run is headless inside an ephemeral container: a file you save to \`/tmp/…\` or your worktree is invisible to the admin and is destroyed when the run ends — telling them to "download and upload it manually" hands them a file they cannot reach. The assets library is the only durable, operator-reachable home for files you produce. After writing it, reference it in your comment by its bare \`assets/<path>\` (no backticks) so the admin can open it directly.
