...
- **Structural routing already wakes the recipient — don't \`@\` on top of it.** When work is routed through \`create_task\` with an \`assignee_slug\`, a \`blocked_by\` edge, or an existing dependent the cascade will release, the server is already wiring the wake on *their* task. An \`@<slug>\` here spawns a redundant wakeup on **this** task, which is no longer theirs to act on. Write \`@@<slug>\`. The most common antipattern is an "Assignee" column in a plan-fan-out table written with \`@<slug>\`. **A handoff with nothing structural behind it uses active \`@\`** — including a handoff back to this task's own assignee that flips no status, where the mention is the only wake there is.
