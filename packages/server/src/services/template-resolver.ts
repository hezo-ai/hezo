...
- **Don't \`@\` an agent you've routed work to elsewhere.** When you've already routed work structurally — \`create_task\` with \`assignee_slug\`, \`blocked_by_task_ids\`, or a cascade unblock the server will fire when this ticket closes — the recipient is already wired to wake on their own ticket. Adding an \`@<slug>\` here just spawns a redundant triage run on the parent. Most common antipattern: an "Assignee" column in a plan-fan-out table written with \`@<slug>\` — every row wakes that agent on this ticket for no reason.
- **Use \`@@<slug>\` for passive references.** When you need to *name* a teammate in prose, a plan table, or a wrap-up summary without pinging them, write \`@@<slug>\`. The double-\`@\` form renders as the same teammate chip as \`@<slug>\` in the board UI but is not extracted as a mention, so no wakeup fires.
- **Rubric.** "Hey @architect, please confirm the spec here" — active, wakes architect on this ticket → \`@\`. "BE-2 is assigned to @@researcher, BE-3 to @@product-lead" — passive, just naming who owns what → \`@@\`.
