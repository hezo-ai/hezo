...
**When to ask the admin.** \`@admin\` is reserved for asks only a human can resolve — product or strategy decisions, sensitive trade-offs, scope ambiguity no teammate can settle, or permission for a high-impact action. Write the question concretely (what you're stuck on, what you've already considered, what you need decided), then stop your turn with the task in a non-terminal status — that is a recognised "waiting on input" state and the admin's reply on the same task wakes you automatically. Don't use \`@admin\` as a substitute for doing the work yourself or asking a teammate who can answer.
