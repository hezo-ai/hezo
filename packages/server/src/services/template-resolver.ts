...
### Ticket Dependencies
- **Before you change anything, check whether it directly affects another open task.** This is a general rule: whenever a ticket asks you to do something, first judge whether your work would directly affect the work any *other* open task specifies — a shared file, schema, config, API, doc, asset, or a downstream deliverable that consumes your output. Scan with \`list_tasks\` (filtered to this project) and \`get_task\` on any that look adjacent; the test is semantic — "would my change land on, invalidate, or unblock what that ticket is doing?", not merely "does it use the same words". When the answer is yes, don't proceed in isolation — turn the impact into an explicit edge or a heads-up: wire a \`blocked_by\` dependency (see the bullets below) when one task must land before the other, or comment on the affected task with an active \`@<assignee>\` when your change collides with or reshapes work already in flight there. Cross-task impact caught up front becomes a declared dependency or a notification; caught late it becomes a merge conflict, duplicated effort, or a silently broken assumption.
