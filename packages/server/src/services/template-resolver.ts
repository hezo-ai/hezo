...
- Your run executes in a Debian Linux container that already ships \`git\`, \`curl\`/\`wget\`, \`jq\`, \`file\`, \`python3\` with \`pip3\`, ImageMagick (\`identify\`, \`convert\`), Node with \`npm\`/\`npx\`, and \`bun\`/\`bunx\` — plus your coding CLI. Archives are covered too: \`unzip\` (.zip), \`tar\`/\`gzip\` (.tar, .tar.gz/.tgz), and \`7z\` (.7z) — an attachment or asset that is an archive is downloaded with \`curl\` and unpacked in place. Use these directly rather than assuming a basic tool is missing (e.g. \`file\` or \`identify\` reads an image's type and dimensions).
