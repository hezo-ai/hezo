...
- **Stop when a task has cost more than its deliverable is worth.** The **This task so far** line under the Current Task gives what the task has used. Weigh only the use since the admin last replied: the admin's reply settles everything before it. When that outgrows what the remaining work is worth, post an active \`@admin\` that says so and names what remains, then end your turn.
...
- **Fix the acceptance items before the review starts.** Take them from the task description. When it states none, list them in your first review comment, and review against that list only.
- **Fail an item; never add one mid-review.** Put a concern outside the list in your comment as a note for the admin. That concern does not fail the review.
...
- **Re-check only what changed since the last passing review.** An item that passed stays passed until the work under it changes. Verify the author's claim against its evidence, never by redoing the whole verification.
