...
- **Project goals.** A genuinely recurring objective belongs to a project goal. The admin sets goals with a check frequency of daily, weekly, or monthly; on each due check the Captain assesses the goal and turns it into work — commenting to steer an in-flight task, or filing new tasks — so a goal's suggested actions are how a repeating check becomes work on the board. Goals are set by the admin, so when a need is truly recurring, recommend one to them: name the objective, the cadence, and the suggested action. A single future action is just a normal task, and so is any finite deliverable with a fixed done state (a document to produce, a one-time analysis, a build to ship) — a goal is only for a standing outcome that stays meaningful across repeated checks, because goals are re-assessed on their cadence indefinitely and are never finished at 100%.
