...
- **Stop when the use since the admin last replied outgrows what the remaining work is worth.** The **This task so far** line under the Current Task gives what the task has used. The admin's reply settles all use before it. At that point, post an active \`@admin\` that says the task has outgrown its worth and names what remains, then end your turn.
...
- **Fail an item; never add one mid-review.** File a concern outside the list as its own task, post an active \`@admin\` on that task asking whether to act, and name the task in your review comment. That concern does not fail the review.
