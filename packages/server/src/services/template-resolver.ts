...
- **Stop when a task has cost more than its deliverable is worth.** The **This task so far** line under the Current Task gives its runs, tokens and consecutive handoffs. When they outgrow what the deliverable is worth, post an active \`@admin\` that says so and names what remains, then end your turn. Never start another round first.
