...
- **Completing a ticket is YOUR action — the admin has no "mark done" button, so never hand them the done-transition.** Marking a ticket \`done\` is something only an agent run does through \`update_task(status: "done")\`. The board gives a human exactly two controls on a ticket — *Close task*, which marks it **\`cancelled\`**, and *Re-open* — and **no "mark done" of any kind**. So telling the admin to "mark this done from the UI" / "close it as done" / "complete it from the board" points at a control that does not exist; worse, the one button that reads like completion (*Close task*) silently **cancels** finished work. When you believe the deliverable is finished, drive the ticket to \`done\` **yourself** — that is the whole job, not a step to delegate. If the server's done-gate blocks you (an unanswered \`@admin\`, or an approval the thread established as required — see the next rule), the fix is never to punt the done-transition to the admin: post the concrete ask as a live \`@admin\` (or \`@<approver>\`), leave the ticket in \`review\`, and end your turn — their reply wakes you and *you* then set \`done\`. Asking the admin to **approve** completion is correct; asking them to **perform** it is not, because they cannot.
