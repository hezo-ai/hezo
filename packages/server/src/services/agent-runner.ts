			...
			if (currentUsage) {
				await deps.db.query(
					...
					[
						...
						currentUsage.cacheReadTokens ?? null,
						currentUsage.cacheCreationTokens ?? null,
	...
	await db.query(
		`UPDATE heartbeat_runs
		 SET status = $1::heartbeat_run_status,
		     ...
		     usage_partial = COALESCE($9, usage_partial)
		 WHERE id = $10`,
		[
			update.status,
			update.exitCode,
			...
			update.usage?.cacheReadTokens ?? null,
			update.usage?.cacheCreationTokens ?? null,
