...
describe('in-memory store', () => {
	test('returns null before any position is set', () => {
		expect(getFabPosition('chat')).toBeNull();
	});

	test('stores a position and the latest write wins', () => {
		setFabPosition('chat', { xFrac: 0.25, yFrac: 0.5 });
		expect(getFabPosition('chat')).toEqual({ xFrac: 0.25, yFrac: 0.5 });
		setFabPosition('chat', { xFrac: 0.9, yFrac: 0.1 });
		expect(getFabPosition('chat')).toEqual({ xFrac: 0.9, yFrac: 0.1 });
	});

	test('clamps fractions to [0,1] on write and rejects non-finite values', () => {
		setFabPosition('chat', { xFrac: -3, yFrac: 42 });
		expect(getFabPosition('chat')).toEqual({ xFrac: 0, yFrac: 1 });
		// Non-finite values are garbage, not "very far right" — they zero out.
		setFabPosition('chat', { xFrac: Number.NaN, yFrac: Number.POSITIVE_INFINITY });
		expect(getFabPosition('chat')).toEqual({ xFrac: 0, yFrac: 0 });
	});

	test('reset clears every stored position', () => {
		setFabPosition('chat', { xFrac: 0.5, yFrac: 0.5 });
		resetFabPositions();
		expect(getFabPosition('chat')).toBeNull();
	});
});
