...
export function GitHubDeviceFlowDialog({
	...
	useEffect(() => {
		...
		setStatusMessage('Requesting a device code from GitHub…');
		startDeviceFlowRef.current
			.mutateAsync(scopesRef.current)
			.then((flow) => {
				setDeviceFlow(flow);
				try {
	...
	useEffect(() => {
		...
		setStatusMessage('Waiting for you to authorize on GitHub…');
	...
	return (
		<Dialog.Root open={open} onOpenChange={onOpenChange}>
			<Dialog.Portal>
				...
				<Dialog.Content
					data-testid="github-device-flow-dialog"
				...
				>
					...
					<Dialog.Description className="text-sm text-text-muted mb-4">
						Authorize Hezo on GitHub by entering the code below on the page that just opened.
					</Dialog.Description>
