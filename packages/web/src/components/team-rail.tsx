...
export function TeamRail() {
	...
	const activeTeamId = (params as Record<string, string>).teamId;
	const navigate = useNavigate();
	const approvalsQuery = useAllPendingApprovals(teams?.map((c) => c.slug) ?? []);
	...
	return (
		<aside className="w-[60px] shrink-0 border-r border-border bg-bg-subtle flex flex-col items-center py-3 gap-2 overflow-y-auto">
			...
			{teams?.map((team) => {
				const isActive = team.slug === activeTeamId;
				return (
					<Link
						key={team.id}
						to="/teams/$teamId"
