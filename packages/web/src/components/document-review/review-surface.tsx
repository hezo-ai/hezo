...
/**
 * The slice of a review comment the surface needs. Structurally satisfied by
 * both the doc rows (`DocReviewComment`) and the asset rows (`ReviewComment`
 * from @hezo/shared). A `null` quote (whole-asset comment) never renders a
 * highlight — those live in the hosting surface's own list UI.
 */
...
export interface ReviewSurfaceProps {
	comments: SurfaceComment[] | undefined;
	/** A create is in flight (disables the editor's Save). */
	saving: boolean;
	/** Controlled active comment (the one open in the editor / highlighted). */
	activeId: string | null;
	onActiveIdChange: (id: string | null) => void;
	/** Persist a new anchored comment; throw to surface the error as a toast. */
	onCreate: (anchor: ReviewAnchor, text: string) => Promise<void>;
	onUpdate: (commentId: string, text: string) => void;
	onDelete: (commentId: string) => void;
	/**
	 * Render the reviewable content (markdown prose, highlighted plain text, …).
	 * MUST be referentially stable across the host's renders (useCallback) — the
	 * surface memoizes on it so its own hover/selection churn never re-renders
	 * the content, which would swap every mark's DOM node mid-click.
	 */
	renderContent: (
 ...
 * margin icons that scroll with the content. Data and persistence are injected
 * — the host wires its own query/mutation hooks (docs vs assets).
 */
export function ReviewSurface({
	...
	const annotations = useMemo<ReviewAnnotation[]>(
		() =>
			(comments ?? [])
				.filter((c): c is SurfaceComment & { quote: string } => c.quote !== null)
				.map((c) => ({ id: c.id, quote: c.quote, occurrence: c.occurrence })),
		[comments],
	);
	const activeComment: SurfaceComment | null =
	...
	// ---- margin icon layout ----------------------------------------------------
	// biome-ignore lint/correctness/useExhaustiveDependencies: renderContent/activeId reposition marks
	useLayoutEffect(() => {
		const wrapper = wrapperRef.current;
		const contentEl = contentRef.current;
		if (!wrapper || !contentEl || annotations.length === 0) {
			setIconLayout((prev) => (prev.length === 0 ? prev : []));
			return;
		}
		const compute = () => {
			const wRect = wrapper.getBoundingClientRect();
			const entries: IconEntry[] = annotations.map((c) => {
	...
	}, [annotations, renderContent, activeId]);

	// ---- save / delete ----------------------------------------------------------
	async function handleSave(text: string) {
		if (draft) {
			try {
				await onCreate(draft.anchor, text);
				setDraft(null);
			} catch (e) {
	...
	// Memoize the rendered content so hover/selection/draft state changes in
	// this wrapper don't re-run the renderer: a re-render swaps every mark's
	...
	const prose = useMemo(
		() => renderContent(annotations, openExisting, activeId),
		[renderContent, annotations, openExisting, activeId],
	);
