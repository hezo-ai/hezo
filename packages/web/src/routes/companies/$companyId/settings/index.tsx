		...
		<div className="grid grid-cols-[160px_1fr] gap-6">
			...
			<div className="space-y-8">
				...
				<div id="settings-ai-providers">
					<AiProvidersSection companyId={companyId} />
				</div>
...
function AiProvidersSection({ companyId }: { companyId: string }) {
	const { data: configs } = useAiProviders(companyId);
	const createProvider = useCreateAiProvider(companyId);
	const deleteProvider = useDeleteAiProvider(companyId);
	const verifyProvider = useVerifyAiProvider(companyId);
	...
	function getConfigForProvider(provider: AiProvider) {
		return configs?.find((c) => c.provider === provider);
	}

	return (
		<section>
			<SectionHeader title="AI providers" desc="API keys for AI coding agents." />
