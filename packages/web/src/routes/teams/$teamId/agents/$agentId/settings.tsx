...
function AgentSettingsPage() {
	...
	const { data: promptDoc, isLoading: isPromptLoading } = useAgentSystemPrompt(teamId, agentId);
	const { data: revisions } = useAgentSystemPromptRevisions(teamId, agentId);
	const restorePrompt = useRestoreAgentSystemPrompt(teamId, agentId);
	const updateAgent = useUpdateAgent(teamId, agentId);
	const disableAgent = useDisableAgent(teamId);
	const enableAgent = useEnableAgent(teamId);
		...
		<div>
			...
			<form onSubmit={handleSave} className="flex flex-col gap-4">
				...
				<div className="flex flex-col gap-1.5">
					...
					>
						Auto-generated by the CEO from the team's structure and system prompts. Injected into
						this agent's system prompt at the start of every run.
					</p>
				</div>
...
export const Route = createFileRoute('/teams/$teamId/agents/$agentId/settings')({
