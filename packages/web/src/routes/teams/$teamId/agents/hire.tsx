...
function HireAgentPage() {
	...
	const onboardAgent = useOnboardAgent(teamId);
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		if (result.issue) {
			...
			navigate({
				...
				params: { teamId, agentId: result.agent.slug },
			});
		} else {
			navigate({ to: '/teams/$teamId/agents', params: { teamId } });
		}
	}
		...
		<div>
			<form onSubmit={handleSubmit} className="flex flex-col gap-5">
				...
				<div className="flex justify-end gap-2 pt-4 border-t border-border">
					<Link to="/teams/$teamId/agents" params={{ teamId }}>
...
export const Route = createFileRoute('/teams/$teamId/agents/hire')({
