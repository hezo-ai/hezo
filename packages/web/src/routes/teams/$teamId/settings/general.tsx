...
import { DEFAULT_SUBTASK_PAGE_SIZE, useTeam, useUpdateTeam } from '../../../../hooks/use-teams';
		...
		<div className="flex flex-col gap-4 md:grid md:grid-cols-[160px_1fr] md:gap-6">
			...
			<div className="space-y-8">
				<div id="settings-general">
					<GeneralSection teamId={teamId} />
				</div>
				<div id="settings-automations">
					<AutomationsSection teamId={teamId} />
				</div>
...
function GeneralSection({ teamId }: { teamId: string }) {
			...
			<div className="space-y-3 max-w-md">
				...
				{team?.description && (
					<div>
						...
						<div className="text-[13px] text-text-muted">{team.description}</div>
					</div>
				)}
			</div>
		</section>
...
function AutomationsSection({ teamId }: { teamId: string }) {
	const { data: team } = useTeam(teamId);
...
function SecretsSection({ teamId }: { teamId: string }) {
