...
| A primitive more than one app draws with (dialog, confirmation, button, input, keycap, shortcut binding) | `@hezo/ui` (`packages/ui/src/`) - the app's `components/ui/` files are pass-throughs, and the two dialogs are wrappers supplying the translated labels. Never resolve copy or read a raw `var(--token)` inside the package: either makes it unimportable, silently |
