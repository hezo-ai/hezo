...
| "Is this wakeup a person answering the agent, or the system pinging it?" | `DISPATCH_SUPPRESSION_EXEMPT_SOURCES` (`services/no-work-backoff.ts`) - a wake carrying an answer needs a source on that list, and `createWakeup` promotes a merged row onto it rather than letting a coalesce demote it |
