...
- **Streaming responses** (SSE etc.): the proxy does not buffer or modify response bodies — the upstream response is piped straight back to the client. This is what carries a Streamable-HTTP MCP server's server→client channel (e.g. `api.githubcopilot.com/mcp/`). Because such a stream stays open for the whole run, the proxy **tracks every accepted/bridged socket and every in-flight upstream request** and severs them on `releaseRunProxy`, so the proxy→upstream socket is not leaked. Under Bun, `closeAllConnections()` reaches neither a hijacked CONNECT socket nor an in-flight streamed response, and aborting the `ClientRequest` alone does not drop its socket — so the run must destroy them itself. Teardown completion (`closeServerWithDeadline`) is gated on the **listening handle releasing**, not on `server.close()`'s drain callback: under Bun that callback never fires for an `https.Server` with a still-live accepted connection (it parks the full deadline), but the listening port is freed promptly, so resolving on `server.listening` going false returns at once and the loopback port is immediately reusable. The deadline now only warns if the handle is genuinely still bound after it elapses.
