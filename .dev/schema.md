...
| `task_comments` | Thread entries. Polymorphic via `content_type` + `content` JSONB. Includes execution-type comments auto-created when agent runs complete. Each row has a `public_id` — a per-task creation-timestamp slug (`YYYYMMDDHHMMSS` UTC, `-N` suffix on same-second collisions, assigned by a `BEFORE INSERT` trigger) used for `#comment-<public_id>` deep-links and `<TASK-ID>#comment-<public_id>` mention links; the UUID `id` stays the key for FKs/API. An `embedding vector(384)` (HNSW cosine index `idx_comments_embedding`) is backfilled for `text` comments only (system/run/action comments are skipped) so human/agent discussion surfaces in `semantic_search`. | belongs to task |
