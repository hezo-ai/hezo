...
- **`bun run test:live` runs every `test/live/` fixture, and they do not all cost the same thing.** The Daytona fixture bills sandboxes; `group-convergence.live.test.ts` bills model tokens instead - about 60 Messages calls on `HEZO_ANTHROPIC_API_KEY`, measuring whether a group room still holds several reads ten turns in. `HEZO_CONVERGENCE_ROUNDS=1 HEZO_CONVERGENCE_CONTROL=0` is the three-call smoke run; `HEZO_CONVERGENCE_DUMP=<dir>` keeps the transcripts. Each fixture self-skips, by name, on the credential it needs.
