...
- **`bun run test:live` runs every `test/live/` fixture, and they do not all cost the same thing.** The Daytona fixture bills sandboxes; `group-convergence.live.test.ts` bills model tokens instead - about 60 Messages calls, measuring whether a group room still holds several reads ten turns in. It runs on whichever **Claude-Code-bound** provider has an api key (`HEZO_ANTHROPIC_API_KEY`, `HEZO_DEEPSEEK_API_KEY`, `HEZO_ZAI_API_KEY`), since those all serve the Anthropic Messages wire, and reads the root and auth header off `PROVIDER_RUNTIME_ADAPTERS`. `HEZO_CONVERGENCE_ROUNDS=1 HEZO_CONVERGENCE_CONTROL=0` is the three-call smoke run; `HEZO_CONVERGENCE_DUMP=<dir>` keeps the transcripts, and its doc comment carries what the first runs measured. Each fixture self-skips, by name, on the credential it needs.
