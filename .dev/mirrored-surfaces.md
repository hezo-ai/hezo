...
| The agent writing register (ASD-STE100 + Zinsser) | `SHARED_INSTRUCTIONS` § *How You Write*, `CHAT_SHARED_INSTRUCTIONS` § *How You Write* (a chat turn gets none of the former), and `.dev/writing-agent-prompts.md` - one rule at three reaches | the `toContain` strings assert each is present; **agreement between them is on you** |
| The independent-judgement rule (a teammate's output is evidence, not a verdict) | `SHARED_INSTRUCTIONS` § *Think for Yourself* and the `TEAM_GROUP_GUIDE` room bullets (a group turn gets none of the former), plus `INDEPENDENCE_LEADS` in `group-convergence.live.test.ts`, which quotes the three bullet leads to delete them for its control arm | the `toContain` strings assert each is present, and the control arm throws when a quote stops matching; **agreement between the two surfaces is on you** |
...
| A new `WakeupSource` value | the `wakeup_source` Postgres enum (a migration), `DISPATCH_SUPPRESSION_EXEMPT_SOURCES` if it carries an answer rather than a system ping, `CONVERSATIONAL_SOURCES` if an agent can raise it to hand work to another, and a `runTrigger.*` label in all twelve catalogs - without the label the run list calls it "Unknown trigger" | `run-trigger.test.ts` covers the label; **nothing for the others** |
...
| `CODEX_VERSION` moved | `CODEX_CLI_VERSION` (`runtime-adapters/codex.ts`), the client version subscription model lists are read for | `agent-cli-pins.test.ts` |
