...
| The independent-judgement rule (a teammate's output is evidence, not a verdict) | `SHARED_INSTRUCTIONS` § *Think for Yourself* and the `TEAM_GROUP_GUIDE` room bullets (a group turn gets none of the former) | the `toContain` strings assert each is present; **agreement between them is on you** |
