...
**Runtime-agnostic logic is shared, not reimplemented per adapter** — every engine runs the identical in-container `/proc` scan and kill scripts (`sandbox/proc-scripts.ts`), differing only in the transport that carries them. Same for the endpoint map (`sandbox/endpoints.ts`), file access (`sandbox/files.ts`) and the exec handle (`sandbox/handle.ts`).
