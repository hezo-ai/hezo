...
- **Agents organised like a company** — a global [CEO and Coach](./docs/concepts/roles-and-coordination.md) above a per-team Captain and workers; [chat with the CEO](./docs/concepts/roles-and-coordination.md#chatting-with-the-ceo) to scope and hire, while the [Coach](./docs/concepts/roles-and-coordination.md#the-coach) reviews finished work.
- **Teams & projects** — [one team per project](./docs/concepts/projects-and-teams.md), each with its own roster, budget, and container; start from a [template](./docs/concepts/projects-and-teams.md#team-templates), [hire and customize agents](./docs/concepts/hiring-and-agents.md), and [snapshot a team](./docs/concepts/projects-and-teams.md#reusing-a-team-setup) to reuse.
- **Autonomous task execution** — a [task board](./docs/concepts/tasks.md) with per-task rules and progress summaries; agents wake on a [heartbeat](./docs/getting-started/first-project.md#3-watch-the-team-work) to pick up work and request [approvals](./docs/getting-started/first-project.md#4-stay-in-control) for consequential actions. Long tasks survive run limits — work is auto-pushed and re-queued.
- **Steer by goals** — set [goals and a progress view](./docs/concepts/goals.md); the Captain re-checks each goal on a schedule, updating progress, health, and status.
- **Your models, your spend** — [bring your own providers](./docs/ai-models.md) (Claude, ChatGPT, Gemini, Kimi, DeepSeek, Z.ai) via each one's native runtime; [give any agent its own model](./docs/ai-models.md#give-an-agent-its-own-model) and set [budget caps](./docs/concepts/budgets-and-costs.md) that pause and auto-resume runs.
- **Secure by design** — agents see [placeholders, not secrets](./docs/security/secret-protection.md), [encrypted at rest](./docs/security/master-key.md) behind one master key, each in a [sandboxed container](./docs/security/container-isolation.md), with [signed, verified git commits](./docs/security/git-and-verified-commits.md) and an [append-only audit trail](./docs/security/activity-log.md).
- **Self-improving teams** — the [Coach](./docs/concepts/coach-and-self-improving-teams.md) reviews each finished task and writes durable **learned rules** back onto the agents — no manual prompt-tuning.
- **Knowledge & memory** — versioned [documents](./docs/concepts/documents-and-memory.md), reusable [skills](./docs/concepts/skills.md), and agent prompts with inline comments and [full history and one-click restore](./docs/concepts/documents-and-memory.md#version-history); plus [CEO chatbox memory](./docs/concepts/documents-and-memory.md#chatbox-memory), [assets in folders](./docs/concepts/assets.md), and [full-text search](./docs/concepts/search.md).
- **MCP, in and out** — a [built-in MCP server](./docs/mcp/hezo-mcp-server.md) to drive teams and tasks from any client, plus [external MCP servers](./docs/mcp/connecting-mcp-servers.md) scoped per instance, team, or project.
- **Your data, your storage** — an [embedded Postgres](./docs/concepts/your-data.md) with nothing external to run, or your own [hosted Postgres](./docs/deployment/configuration.md) (14+); keep [assets](./docs/concepts/assets.md#where-asset-files-live) on local disk or in any [S3-compatible bucket](./docs/deployment/configuration.md#storing-assets-in-s3-compatible-object-storage), with data-preserving upgrades.
- **Self-hosted & easy to run** — a [single binary](./docs/getting-started/installation.md), [deployable anywhere Docker runs](./docs/deployment/self-hosting.md) or via a [one-click cloud-init snippet](./docs/deployment/one-click.md), with [secure remote access](./docs/deployment/secure-remote-access.md), [safe-rollback backups](./docs/deployment/backup-and-recovery.md), [in-app self-update](./docs/deployment/self-hosting.md#updating), and a mobile-first web app.
